...
3. Clear stale cached wheels: run `./repo.sh uv -- cache clean usd-validation-nvidia` (or `.\repo.bat` on Windows)
4. Find the built wheel in `dist/` — it matches `usd_validation_nvidia-*.whl`. On Linux use `ls dist/usd_validation_nvidia-*.whl | tail -1`; on Windows use `Get-ChildItem dist\usd_validation_nvidia-*.whl | Select-Object -Last 1 -ExpandProperty Name`
