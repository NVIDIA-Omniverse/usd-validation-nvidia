...
The package contains the validation engine and built-in validators. It is commonly used with profile definitions
generated by `usd-profiles-nvidia`; profile packages or custom plugins must be installed in the same Python
environment as the engine so their entry points are discovered.
