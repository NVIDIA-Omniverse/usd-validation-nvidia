...
The package contains the validation engine and built-in validators. It is commonly used with profile definitions generated by `usd-profiles-nvidia`; profile packages or custom plugins must be installed in the same Python environment as the engine so their entry points are discovered.
...
- Asset validation and scope selection: `skills/validate-asset/SKILLS.md`
- JSON report interpretation: `skills/reading-json-output/SKILLS.md`
- Mapping failures to fixes: `skills/fixing-validation-failures/SKILLS.md`
- CI integration: `skills/ci-validation/SKILLS.md`
- Profile spec authoring and code generation: `skills/profile-codegen/SKILLS.md`
- Custom rules, profiles, and entry-point plugins: `skills/extending-validation/SKILLS.md`

If multiple skills seem relevant, start with the narrowest skill that matches the user request, then layer in adjacent skills only when the workflow needs them.
