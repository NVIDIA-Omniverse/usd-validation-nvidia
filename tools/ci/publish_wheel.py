...
def get_version_from_wheel(wheel_name):
    ...
    match = re.search(r"^omniverse_asset_validator-([^-]+)-", wheel_name)
    if match:
...
def register_release_with_kitmaker(wheel_filename: str, version: str, dry_run: bool) -> bool:
    ...
    payload = {
        "project_name": "omniverse-asset-validator",
        "payload": [
            {
                "pic": "miguelh@nvidia.com",
                "job_type": "wheel-release-job",
                "publish_to": "both_devzone_pypi",
                "url": f"https://urm.nvidia.com/artifactory/ct-omniverse-pypi/omniverse-asset-validator/{version}/{wheel_filename}",
                "size": "small",
                "upload": not dry_run,
            }
        ],
