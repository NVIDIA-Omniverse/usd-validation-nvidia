...
from usd_validation_nvidia import ComplianceChecker, LocalUriResolver, ParameterMapping, ValidationStats


class ComplianceCheckerTest(unittest.IsolatedAsyncioTestCase):
    def test_constructor_normalizes_none_defaults(self):
        checker = ComplianceChecker(stats=None, parameters=None, resolver=None)

        self.assertIsInstance(checker.stats, ValidationStats)
        self.assertIsInstance(checker.parameters, ParameterMapping)
        self.assertIsInstance(checker.resolver, LocalUriResolver)
