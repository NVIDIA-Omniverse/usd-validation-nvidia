...
class ValidationParserTest(unittest.TestCase):
    ...
    def test_help(self):
        ...
        self.assertIn("Experimental: maximum process workers", stdout)
    ...
    def test_asset(self):
        ...
        self.assertEqual(args.process, 0)
    ...
    def test_verbose(self):
        parser = create_validation_parser()
        args = parser.parse_args(["-v", "asset.usda"])
        self.assertEqual(args.verbose, 1)

    def test_very_verbose(self):
        parser = create_validation_parser()
        args = parser.parse_args(["-vv", "asset.usda"])
    ...
    def test_process(self):
        parser = create_validation_parser()
        args = parser.parse_args(["--process", "4", "asset.usda"])
        self.assertEqual(args.process, 4)
    ...
    def test_process_negative(self):
        ...
        with redirect_stdout(f), redirect_stderr(g):
            with self.assertRaises(SystemExit):
                parser.parse_args(["--process", "-1", "asset.usda"])

        self.assertIn("invalid non-negative int value: '-1'", g.getvalue())
...
class ValidationArgsTest(unittest.TestCase):
    ...
    def test_create_engine_processes(self):
        parser = create_validation_parser()
        args = parser.parse_args(["--process", "4", "asset.usda"])
        args = ValidationArgsExec(args)
        engine = args.create_engine()
    ...
    def test_run_validation_not_found(self):
        ...
        with self.assertLogs(level="INFO") as cm:
            parser = create_validation_parser()
            args = ValidationArgsExec(parser.parse_args(["-v", "--rule", "TypeChecker", url]))
            args.run_validation()
    ...
    def test_run_validation_report_exports_are_independent(self):
        url = get_url("helloworld.usda")

        with TemporaryDirectory() as tmp:
            json_output = pathlib.Path(tmp) / "ok.json"
            with (
                redirect_stdout(io.StringIO()),
                mock.patch(
                    "usd_validation_nvidia._cli.export_csv_file",
                    side_effect=OSError(13, "blocked"),
                ),
                self.assertLogs("usd_validation_nvidia._cli", level="ERROR"),
            ):
                parser = create_validation_parser()
                args = ValidationArgsExec(
                    parser.parse_args(
                        [
                            "--rule",
                            "TypeChecker",
            ...
            self.assertEqual(json.loads(json_output.read_text())["status"], "PASS")
    ...
    def test_cli(self):
        ...
        self.assertIn("https://nvidia-omniverse.github.io/usd-validation-nvidia/", stdout)
        self.assertIn("--rule", stdout)
        self.assertIn("--fix", stdout)
        self.assertIn("--predicate", stdout)
        self.assertIn("--verbose", stdout)
    ...
    def test_cli_version_ok(self):
        result = subprocess.run(
            [sys.executable, "-m", "usd_validation_nvidia", "--version"],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
class AutoDetectionTest(unittest.TestCase):
    ...
    def test_auto_detect_no_rules_profile_counts_as_matched(self):
        ...
        try:
            url = get_url("helloworld.usda")
            parser = create_validation_parser()
            exec = ValidationArgsExec(parser.parse_args(["-v", url]))
            with self.assertLogs(level="INFO") as cm:
                exec.run_validation()
