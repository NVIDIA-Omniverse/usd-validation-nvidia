...
class IssueJSONEncoderTest(unittest.TestCase):
    ...
    def test_issue_json_includes_unowned_issues(self):
        results = Results(
            asset="test.usd",
            issues=[Issue(severity=IssueSeverity.ERROR, message="unowned error")],
        )

        result = json.loads(json.dumps(ResultsList([results]), cls=IssueJSONEncoder))

        self.assertNotIn("issues", result)
        self.assertIsNone(result["rules"][0]["rule"])
        self.assertEqual([issue["message"] for issue in result["rules"][0]["issues"]], ["unowned error"])
    ...
    def test_export_json_file_writes_iterencode_chunks(self):
        # Given
        json_file = MagicMock(spec=io.TextIOBase)
        json_file.__enter__.return_value = json_file
        results = Results(
            asset="test.usd",
            issues=[Issue(severity=IssueSeverity.FAILURE, rule=MyRuleChecker, message="failure")],
        )
        ...
        # When
        with patch("builtins.open", return_value=json_file):
            export_json_file("results.json", ResultsList([results]))

        # Then
        self.assertGreater(json_file.write.call_count, 1)
