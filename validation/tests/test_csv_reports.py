...
class BaseCSVTestMixin:
    ...
    def test_csv_issues(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            csv_data = self.from_([issue1, issue2])
            csv_data.export_csv(csv_path)
            # Then
            self.assertEqual(
                pathlib.Path(src_csv).read_text(),
        ...
        with TemporaryDirectory() as tmp:
            csv_path: str = os.path.join(tmp, "issues.csv")
            csv_data = self.from_(IssuesList([issue1, issue2]))
            csv_data.export_csv(csv_path)
            # Then
            self.assertEqual(
                pathlib.Path(src_csv).read_text(),
    ...
    def test_csv_results_list(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            csv_data = self.from_(results_list)
            csv_data.export_csv(csv_path)
            # Then
            self.assertEqual(
                pathlib.Path(src_csv).read_text(),
    ...
    def test_csv_get_data_dict(self):
        ...
        # When
        csv_data = self.from_(results1)
        csv_data.append_column("Test", ["1", "2"])
    ...
    def test_csv_export_with_delimiters(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            csv_data = self.from_(IssuesList([issue_with_comma_delimiter]))
            csv_data.export_csv(csv_path)
            # Then
            self.assertEqual(
                pathlib.Path(src_csv).read_text(),
    ...
    def test_csv_export_with_empty_cells(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            csv_data = self.from_(IssuesList([issue]))
            csv_data.export_csv(csv_path)
            # Then
            self.assertEqual(
                pathlib.Path(src_csv).read_text(),
...
class ColumnCSVTableTest(BaseCSVTestMixin, unittest.TestCase):

    def from_(self, value):
        return ColumnCSVTable.from_(value)


class RowCSVTableTest(BaseCSVTestMixin, unittest.TestCase):

    def from_(self, value):
        return RowCSVTable.from_(value)


class IssueCSVDataTest(unittest.TestCase):

    def test_issue_csv_data_is_column_csv_table_alias(self):
        self.assertIs(IssueCSVData, ColumnCSVTable)
