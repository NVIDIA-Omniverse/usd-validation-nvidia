...
class LayerIdTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restore_identifier_ok(self):
        # Given
        layer_id = LayerId.from_(self.layer)

        # When
        restored = restore_identifier(layer_id)

        # Then
        self.assertEqual(self.layer, restored)

    async def test_pickle_ok(self):
        # Given
        layer_id = LayerId.from_(self.layer)

        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(layer_id)), layer_id)

    async def test_dedupe_ok(self):
        # When
        layer_id1 = LayerId.from_(self.layer)
        layer_id2 = LayerId(identifier=self.url)

        # Then
        self.assertIs(layer_id1, layer_id2)
...
class StageIdTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restore_identifier_ok(self):
        ...
        # When
        restored = restore_identifier(stage_id)

        # Then
        self.assertEqual(StageId.from_(restored), stage_id)
    ...
    async def test_pickle_ok(self):
        ...
        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(stage_id)), stage_id)

    async def test_dedupe_ok(self):
        # When
        stage_id1 = StageId.from_(self.stage)
        stage_id2 = StageId(root_layer=LayerId(self.url))

        # Then
        self.assertIs(stage_id1, stage_id2)
...
class PrimIdTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restore_identifier_ok(self):
        ...
        # When
        restored = restore_identifier(prim_id)

        # Then
        self.assertEqual(PrimId.from_(restored), prim_id)
    ...
    async def test_pickle_ok(self):
        ...
        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(prim_id)), prim_id)
...
class AttributeIdTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restore_identifier_ok(self):
        ...
        # When
        restored = restore_identifier(attr_id)

        # Then
        self.assertEqual(AttributeId.from_(restored), attr_id)
    ...
    async def test_pickle_ok(self):
        ...
        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(attr_id)), attr_id)
...
class RelationshipIdTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        self.url = get_url("helloworld.usda")
        self.stage = Usd.Stage.Open(self.url)
        self.prim = self.stage.GetPrimAtPath("/World/box_0")
        self.rel = self.prim.GetRelationship("material:binding")

    async def test_restore_identifier_ok(self):
        # Given
        rel_id = RelationshipId.from_(self.rel)

        # When
        restored = restore_identifier(rel_id)

        # Then
        self.assertEqual(RelationshipId.from_(restored), rel_id)
    ...
    async def test_pickle_ok(self):
        ...
        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(rel_id)), rel_id)
...
class PrimvarIdTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restore_identifier_ok(self):
        ...
        # When
        restored = restore_identifier(primvar_id)

        # Then
        self.assertEqual(PrimvarId.from_(restored), primvar_id)
    ...
    async def test_pickle_ok(self):
        ...
        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(primvar_id)), primvar_id)
...
class SchemaBaseIdTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restore_identifier_ok(self):
        ...
        # When
        restored = restore_identifier(schema_base_id)

        # Then
        self.assertEqual(SchemaBaseId.from_(restored), schema_base_id)
    ...
    async def test_pickle_ok(self):
        ...
        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(schema_base_id)), schema_base_id)
...
class FormatDependencyIdTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restore_ok(self):
        ...
        # When
        restored = dep_id.restore()

        # Then
        self.assertEqual(restored, self.dependency)
    ...
    async def test_restore_identifier_ok(self):
        ...
        # When
        restored = restore_identifier(dep_id)
    ...
    async def test_pickle_ok(self):
        ...
        # When / Then
        self.assertEqual(pickle.loads(pickle.dumps(dep_id)), dep_id)
...
class ToIdentifierScalarDispatchTests(unittest.TestCase):
    def test_string_passthrough_ok(self):
        self.assertEqual(to_identifier("/a/dep.usd"), "/a/dep.usd")

    def test_restore_string_ok(self):
        self.assertEqual(restore_identifier("/a/dep.usd"), "/a/dep.usd")

    def test_string_list_passthrough_ok(self):
        self.assertEqual(to_identifier(["/a/dep.usd", "/b/dep.usd"]), ["/a/dep.usd", "/b/dep.usd"])
...
class ToIdentifierListDispatchTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_normalized_to_list_of_identifiers(self):
        # When
    ...
    async def test_empty_list_normalized_to_empty_list(self):
        # When
    ...
    async def test_restore_tuple_ok(self):
        # Given
        identifiers = to_identifier((self.prim1, self.prim2))

        # When
        result = restore_identifier(identifiers, self.stage)

        # Then
        self.assertEqual(result, (self.prim1, self.prim2))
