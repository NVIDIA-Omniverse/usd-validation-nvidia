...
class CapabilitiesRegistryTest(unittest.TestCase):
    ...
    def test_add_capability(self):
        self.registry.add(self.mock_capability)

        self.assertIn(self.mock_capability, self.registry.capabilities)
        self.assertEqual(self.registry.find(self.mock_capability.id), self.mock_capability)

    def test_find_capability(self):
        self.registry.add(self.mock_capability)

        self.assertEqual(self.registry.find("test_capability"), self.mock_capability)
    ...
    def test_singleton(self):
        ...
        registry1.add(self.mock_capability)

        self.assertIsNotNone(registry1.find("test_capability"))
        self.assertIsNotNone(registry2.find("test_capability"))
    ...
    def test_register_capabilities(self):
        self.assertTrue(register_capabilities([self.mock_capability]))
        try:
            self.assertIn(self.mock_capability, self.registry.capabilities)
        finally:
            unregister_capabilities([self.mock_capability])

    def test_register_capabilities_duplicate_returns_false(self):
        register_capability(self.mock_capability)
        try:
            self.assertFalse(register_capabilities([self.mock_capability]))
        finally:
            unregister_capability(self.mock_capability)

    def test_unregister_capabilities(self):
        register_capabilities([self.mock_capability])
        self.assertTrue(unregister_capabilities([self.mock_capability]))
