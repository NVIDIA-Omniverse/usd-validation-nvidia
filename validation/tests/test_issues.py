...
class SuggestionTest(unittest.IsolatedAsyncioTestCase):
    async def test_pickle_ok(self):
        # Given
        layer = Sdf.Layer.CreateAnonymous("helloworld.usda")
        suggestion = Suggestion(callable=print, message="A suggestion", at=[layer])

        # When
        self.assertEqual(pickle.loads(pickle.dumps(suggestion)), suggestion)

    async def test_hashable_and_equals(self):
        def func(a, b):
            pass
    ...
    async def test_dedupe_ok(self):
        ...
        suggestion1 = Suggestion(callable=func, message="A suggestion")
        suggestion2 = Suggestion(callable=func, message="A suggestion")

        self.assertIs(suggestion1, suggestion2)
...
class IssueTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pickle_ok(self):
        ...
        # When
        self.assertEqual(pickle.loads(pickle.dumps(issue)), issue)
