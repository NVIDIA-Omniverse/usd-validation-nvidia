...
def _non_negative_int(arg: str) -> int:
    try:
        value = int(arg)
    except ValueError as e:
        raise argparse.ArgumentTypeError(f"invalid int value: '{arg}'") from e

    if value < 0:
        raise argparse.ArgumentTypeError(f"invalid non-negative int value: '{arg}'")
...
def create_validation_parser() -> argparse.ArgumentParser:
    ...
    - process: Optional. Default 0. Sets ValidationEngine, processes argument.
    ...
    parser.epilog = "See https://nvidia-omniverse.github.io/usd-validation-nvidia/ for more details."
    parser.add_argument(
        "asset",
        metavar="ASSET",
        type=str,
    ...
    parser.add_argument(
        ...
        help=inspect.cleandoc(
            """
            Experimental: maximum process workers to use for multiprocess-safe validation tasks.
            Use 0 to disable process runners.
            """
        ),
    )
    # CSV Output
...
class ValidationNamespaceExec:
    ...
    def log_level(self) -> int:
        """
        Returns:
            The logging level selected by the parsed verbosity.
        """
        verbosity: int = getattr(self._namespace, "verbose", 0)
        if verbosity == 0:
    ...
    def processes(self) -> int:
        """
        Returns:
            The `process` option value.
        """
        return self._namespace.process
    ...
    def _assets_summary(self, results: ResultsList) -> None:
        issues = results.issues().filter_by(self.predicate)
        severity_mapper = Counter()
        for group in issues.group_by(IssueGroupsBy.severity()):
            severity_mapper[group.name] += len(group)

        if severity_mapper[IssueSeverity.FAILURE] or severity_mapper[IssueSeverity.ERROR]:
            summary_level = logging.ERROR
        elif severity_mapper[IssueSeverity.WARNING]:
        ...
        # Group by severity
        logger.log(summary_level, "-" * 128)
        logger.log(summary_level, "Summary per Severity:")
        logger.log(summary_level, f"Failures: {severity_mapper[IssueSeverity.FAILURE]}")
        logger.log(summary_level, f"Warnings: {severity_mapper[IssueSeverity.WARNING]}")
        logger.log(summary_level, f"Errors: {severity_mapper[IssueSeverity.ERROR]}")
        logger.log(summary_level, f"Infos: {severity_mapper[IssueSeverity.INFO]}")

    def _export_csv(self, results: ResultsList, context: ValidationContext | None = None) -> bool:
        try:
            export_csv_file(self.csv_output, results, metadata=context)
        except OSError as e:
            logger.error(f"Failed to write CSV output '{self.csv_output}': {e}")
            return False
        return True

    def _export_json(self, results: ResultsList, context: ValidationContext | None = None) -> bool:
        try:
            export_json_file(self.json_output, results, metadata=context)
        except OSError as e:
            logger.error(f"Failed to write JSON output '{self.json_output}': {e}")
            return False
        return True
    ...
    def _log_validation_context(cls, context: ValidationContext | None) -> None:
        ...
        for f in failed_features:
            ...
            logger.error(f"Failed requirements {failed_reqs} preclude feature {f.feature.id}.")
    ...
    def _log_profile_detection(context: ValidationContext | None) -> None:
        ...
        if context.failed_profiles:
            logger.error("Non-matching profiles:")
            for p in context.failed_profiles:
                failed_codes = [r.code for r in p.failed_requirements]
                logger.error(f"  {p.profile.id} ({p.profile.version}) - FAIL: {failed_codes}")
    ...
    async def _validate(self) -> bool:
        ...
        if self.csv_output:
            outputs_written = self._export_csv(results, context) and outputs_written
        if self.json_output:
            outputs_written = self._export_json(results, context) and outputs_written
        return not has_issues and outputs_written
