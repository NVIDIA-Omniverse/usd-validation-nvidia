...
class ValidationEngine:
    ...
    _enabled_rules: list[type[BaseRuleChecker]] = field(init=False, default_factory=list)
    _disabled_rules: list[type[BaseRuleChecker]] = field(init=False, default_factory=list)
    _enabled_requirements: list[Requirement | RequirementRefProtocol] = field(init=False, default_factory=list)
    _disabled_requirements: list[Requirement | RequirementRefProtocol] = field(init=False, default_factory=list)
    _enabled_capabilities: list[Capability] = field(init=False, default_factory=list)
    _disabled_capabilities: list[Capability] = field(init=False, default_factory=list)
    _enabled_features: list[Feature] = field(init=False, default_factory=list)
    _disabled_features: list[Feature] = field(init=False, default_factory=list)
    _enabled_profiles: list[Profile] = field(init=False, default_factory=list)
    _disabled_profiles: list[Profile] = field(init=False, default_factory=list)
    _tasks: set[asyncio.Task] = field(init=False, default_factory=set)
    _stats: ValidationStats = field(init=False, default_factory=ValidationStats)
    _parameters: list[Parameter] = field(init=False, default_factory=list)
    ...
    async def validate_async(self, asset: AssetType) -> ResultsList:
        ...
        if isinstance(asset, Usd.Stage):
            ...
            return replace(results_list, context=self.build_context(results_list))
    ...
    def _validate(self, asset: AssetType) -> Results:
        ...
        try:
            ...
            return replace(result, context=self.build_context(result))
    ...
    async def _validate_all_async(
        ...
        results_list = replace(results_list, context=self.build_context(results_list))
        return results_list
