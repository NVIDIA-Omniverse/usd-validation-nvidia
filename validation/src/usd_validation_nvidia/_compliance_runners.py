...
from ._context_managers import MAXIMUM_BATCH_SIZE, DelegateContextManager
from ._identifiers import restore_identifier, to_identifier
from ._stats import ValidationStats
from .utils import is_multiprocess_safe
...
class ComplianceCheckerEventType(Enum):
    ...
    def __reduce_ex__(self, protocol: int):
        return ComplianceCheckerEventType.__getitem__, (self.name,)


# TODO: Set frozen=True when Python 3.10 support is dropped.
...
class ComplianceCheckerEvent:
    ...
    def __getstate__(self) -> dict[str, Any]:
        return {
            "type": self.type,
            # TODO: Add serializable state for Tf.Diagnostic and Sdf.ZipFile event payloads.
            "value": to_identifier(self.value),
        }

    def __setstate__(self, state: dict[str, Any]) -> None:
        object.__setattr__(self, "type", state["type"])
        object.__setattr__(self, "value", restore_identifier(state["value"]))
...
class ComplianceCheckerEventRule:
    ...
    def is_multiprocess_safe_task(self) -> bool:
        method: Callable[..., Any] = self.type.value(self.rule)
        return is_multiprocess_safe(method)
...
class AbstractProcessRunner:
    ...
    @staticmethod
    def apply(event_rule: ComplianceCheckerEventRule) -> ApplyProcessResult:
        stats = ValidationStats()
        issue_count = len(event_rule.rule.GetIssues())
        event_rule.apply(stats)
        issues = tuple(event_rule.rule.GetIssues()[issue_count:])
        return ApplyProcessResult(issues=issues, stats=stats)

    def merge(self, event_rule: ComplianceCheckerEventRule, result: ApplyProcessResult) -> None:
        for issue in result.issues:
            event_rule.rule._AddIssue(issue)
...
class SyncInlineRunner(AbstractSyncRunner[ComplianceCheckerEventRule]):
    ...
    def flush(self) -> None:
        if not self.events:
            return
        with DelegateContextManager(), Ar.ResolverContextBinder(self.context), Ar.ResolverScopedCache():
...
@dataclass
class SyncProcessRunner(AbstractProcessRunner, AbstractSyncRunner[tuple[ComplianceCheckerEventRule, Future]]):
    """Starts sync events in a process pool during submit."""

    processes: int = 0
    pool: ProcessPoolExecutor | None = field(init=False, default=None)

    def __post_init__(self) -> None:
        if self.processes < 0:
            raise ValueError("processes must be greater than or equal to 0")
        elif self.processes > 0:
            self.pool = ProcessPoolExecutor(max_workers=self.processes)

    def accepts(self, event_rule: ComplianceCheckerEventRule) -> bool:
        return self.pool is not None and event_rule.is_multiprocess_safe_task() and not event_rule.is_async_task()

    def map(self, event_rule: ComplianceCheckerEventRule) -> tuple[ComplianceCheckerEventRule, Future]:
        if self.pool is None:
            raise RuntimeError("process runner is disabled")
        return event_rule, self.pool.submit(self.apply, event_rule)

    def flush(self) -> None:
        if not self.events:
            return
        events: list[tuple[ComplianceCheckerEventRule, Future]] = self.pop()
        for event_rule, future in events:
            self.merge(event_rule, future.result())
        self.counter += len(events)
...
class AsyncProcessRunner(
    ...
    AbstractAsyncRunner[tuple[ComplianceCheckerEventRule, asyncio.Future]],
):
    """Starts sync events in a process pool during async submit."""
    ...
    def map(self, event_rule: ComplianceCheckerEventRule) -> tuple[ComplianceCheckerEventRule, asyncio.Future]:
        loop: AbstractEventLoop = asyncio.get_running_loop()
        if self.pool is None:
            raise RuntimeError("process runner is disabled")
        return event_rule, loop.run_in_executor(self.pool, self.apply, event_rule)

    async def flush_async(self) -> None:
        if not self.events:
            return
        events: list[tuple[ComplianceCheckerEventRule, asyncio.Future]] = self.pop()
        results: list[ApplyProcessResult] = await asyncio.gather(*(future for _, future in events))
        for (event_rule, _), result in zip(events, results):
            self.merge(event_rule, result)
        self.counter += len(events)
...
class SyncComplianceCheckerRunner(AbstractContextManager):
    ...
    def __post_init__(self, stats: ValidationStats) -> None:
        self.runners = [
            ...
            SyncProcessRunner(stats, processes=self.processes),
...
class AsyncComplianceCheckerRunner(AbstractAsyncContextManager):
    ...
    def __post_init__(self, stats: ValidationStats) -> None:
        self.runners = [
            ...
            AsyncProcessRunner(stats, processes=self.processes),
    ...
    async def close_async(self) -> None:
        await asyncio.gather(*(runner.close_async() for runner in self.runners))
