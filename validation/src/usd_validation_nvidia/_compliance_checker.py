...
_USD_2508_API: bool = Usd.GetVersion() >= (0, 25, 8)


@dataclass(kw_only=True, frozen=True, eq=False, repr=False)
class ComplianceChecker:
    ...
    * "structural" validation that is represented by a set of base rules.
    ...
    _issues: list[Issue] = field(init=False, default_factory=list)
    # Once a package has been checked, it goes into this set.
    _checked_packages: set[str] = field(init=False, default_factory=set)
    _rule_types: set[type[BaseRuleChecker]] = field(init=False, default_factory=set)
    _rules: list[BaseRuleChecker] = field(init=False, default_factory=list)

    def __post_init__(self) -> None:
        object.__setattr__(self, "stats", self.stats or ValidationStats())
        object.__setattr__(self, "parameters", self.parameters or ParameterMapping())
        object.__setattr__(
            self,
            "resolver",
            self.resolver if self.resolver is not None else LocalUriResolver(),
        )
        ...
        try:
            with DelegateContextManager():
                if self._is_old_style_rule(rule_type):
                    ...
                    rule: BaseRuleChecker = rule_type(parameters=self.parameters)
    ...
    def issues(self) -> Sequence[Issue]:
        """
        Returns all issues from all rules.
        """
        issues: list[Issue] = list(self._issues)
        for rule in self._rules:
            if rule_issues := rule.GetIssues():
                issues.extend(rule_issues)
        return tuple(issues)

    @property
    def partial_issues(self) -> Sequence[Issue]:
        """
        Returns partial issues from all rules.
        """
        issues: list[Issue] = list(self._issues)
        for rule in self._rules:
            if rule_issues := rule.GetIssues():
                issues.append(rule_issues[-1])
        return tuple(issues)
    ...
    def check(self, asset: str | Usd.Stage) -> None:
        ...
        if self._rules_check_assets():
            context = self.get_dependency_context(asset)
            asset_dependencies = self._check_dependencies(asset, context)

        with SyncComplianceCheckerRunner(rules=self._rules, stats=self.stats, processes=self.processes) as runner:
    ...
    async def check_async(self, asset: str | Usd.Stage, callback: Callable[[float], None] | None = None) -> None:
        ...
        self._checked_packages.clear()
        # Run checkers
        runner = self._create_async_runner(self._rules, self.stats, processes=self.processes)

        def progress_callback() -> None:
            callback(round(min(runner.counter / num_events, _MAX_PROGRESS), 2))
    ...
    def _create_async_runner(
        cls, rules: list[BaseRuleChecker], stats: ValidationStats, *, processes: int = 0
    ) -> AsyncComplianceCheckerRunner:
        return AsyncComplianceCheckerRunner(rules=rules, stats=stats, processes=processes)
        ...
        with Ar.ResolverContextBinder(usd_stage.GetPathResolverContext()):
            if self._rules_check_assets():
                all_layers, all_assets, unresolved_paths = asset_dependencies or self._check_dependencies(
                    asset, self.get_dependency_context(asset)
                )
    ...
    @staticmethod
    def get_dependency_context(asset: str | Usd.Stage) -> Ar.Context:
        if isinstance(asset, Usd.Stage):
            return asset.GetPathResolverContext()
        return Ar.GetResolver().GetCurrentContext()
    ...
    def _check_asset_format(self, asset_path: str) -> Iterator[ComplianceCheckerEvent]:
        ...
        if asset_format is None:
            ...
            return
        for dependency_path in asset_format.get_dependencies(asset_path, self.resolver):
    ...
    def _check_package(self, package_path) -> Iterator[ComplianceCheckerEvent]:
        ...
        if package_path in self._checked_packages:
            return
        self._checked_packages.add(package_path)
        ...
        if _USD_2508_API:
            zip_file = Sdf.ZipFile.Open(resolved_path)
        else:
    ...
    def _traverse_range(self, primRangeIt, isStageRoot) -> Iterator[ComplianceCheckerEvent]:
        ...
        for prim in primRangeIt:
            ...
            if self.skip_variants or (not isStageRoot and prim == root_prim):
