...
@register_rule("Geometry")
class UsdGeomSubsetFamiliesChecker(UsdValidatorAdapter):
    """
    Validates UsdGeomSubset family membership and indices using OpenUSD's SubsetFamilies validator.
    """

    @classmethod
    def validator_name(cls) -> str:
        return "usdGeomValidators:SubsetFamilies"
    ...
    def _CheckPrim(self, prim: Usd.Prim) -> None:
        ...
        subset_family_names = sorted(UsdGeom.Subset.GetAllGeomSubsetFamilyNames(imageable))
        for subset_family_name in subset_family_names:
            family_subsets = UsdGeom.Subset.GetGeomSubsets(
                imageable,
                elementType="",
                familyName=subset_family_name,
            )
            if not family_subsets:
                continue

            element_type = family_subsets[0].GetElementTypeAttr().Get() or ""
            is_valid, reason = UsdGeom.Subset.ValidateFamily(imageable, element_type, subset_family_name)
            if is_valid:
                continue

            self._AddFailedCheck(
                f"Imageable prim <{prim.GetPath()}> has invalid subset family '{subset_family_name}': {reason}",
                at=prim,
            )


@register_rule("Geometry")
class UsdGeomSubsetParentIsImageableChecker(UsdValidatorAdapter):
    """
    Validates that every UsdGeomSubset has a direct parent prim that is UsdGeomImageable.
    """

    @classmethod
    def validator_name(cls) -> str:
        return "usdGeomValidators:SubsetParentIsImageable"
    ...
    def _CheckPrim(self, prim: Usd.Prim) -> None:
        ...
        if UsdGeom.Imageable(parent_prim):
            return

        self._AddFailedCheck(
            f"GeomSubset <{prim.GetPath()}> has direct parent prim <{parent_prim.GetPath()}> that is not Imageable.",
            at=prim,
        )
