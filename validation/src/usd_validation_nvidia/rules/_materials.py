...
@register_requirements(cap.Requirements.VM_MDL_001)
        ...
        for attribute in prim.GetAttributes():
            ...
            if is_mdl_source_asset and (not attribute_value or not attribute_value.path):
                self._AddFailedCheck(
                    requirement=cap.Requirements.VM_MDL_001,
...
@register_rule("Material")
@register_requirements(cap.Requirements.VM_BIND_001)
    ...
    def CheckPrim(self, prim: Usd.Prim) -> None:
        ...
        for rel in prim.GetRelationships():
            ...
            for spec_layer, spec_path in self._get_out_of_scope_specs(rel):
                self._AddFailedCheck(
                    requirement=cap.Requirements.VM_BIND_001,
...
@register_rule("Material")
@register_requirements(cap.Requirements.VM_PS_001)
class MaterialUsdPreviewSurfaceChecker(BaseRuleChecker):
    ...
    @classmethod
    def write_usd_shade_input_metadata(
        cls, usd_shade_input: UsdShade.Input, sdr_property: Sdr.ShaderProperty
    ) -> None:
        """Set the metadata iff it differs from what is already defined."""
        if cls._validate_usd_shade_input_metadata(usd_shade_input, sdr_property) is not None:
    ...
    @classmethod
    def update_usd_shade_input(cls, usd_shade_input: UsdShade.Input, sdr_property: Sdr.ShaderProperty) -> None:
        """Update the UsdShade.Input from SdrProperty, restore any parameter values and/or connections that have been set."""

        (force_connection_write, _, input_value, connections) = cls._input_value_and_connections_transform(
            usd_shade_input, sdr_property
        )

        if cls._validate_usd_shade_input_value(usd_shade_input, input_value, sdr_property) is not None:
        ...
        if not cls._validate_type_name(sdr_property, sdf_type):
            input_value.value = sdr_property.GetDefaultValue()
            input_value.time_samples = None

        cls.write_usd_shade_input_output_type_name(usd_shade_input, sdf_type)

        if cls._validate_usd_shade_input_connectability(usd_shade_input, sdr_property) is not None:
            ...
            cls.write_usd_shade_input_connectibility(usd_shade_input, connectability)

        cls.write_usd_shade_input_metadata(usd_shade_input, sdr_property)

        if connections and connections[0]:
            cls.write_usd_shade_input_connections(usd_shade_input, connections, force_connection_write)

        cls.write_usd_shade_input_value(usd_shade_input, input_value)

    @classmethod
    def update_usdshade_output(cls, usdshade_output: UsdShade.Output, sdr_property: Sdr.ShaderProperty) -> None:
        """Update the UsdShade.Output from SdrProperty."""
        sdf_type = get_sdf_type_for_shader_property(sdr_property)
        cls.write_usd_shade_input_output_type_name(usdshade_output, sdf_type)

    @classmethod
    def _should_input_be_filtered(cls, shader_input: UsdShade.Input) -> bool:
        # Placeholder
        return False

    @classmethod
    def update_usdshade_shader(cls, usdshade_shader: UsdShade.Shader) -> None:
        ...
        if shader_id not in cls.usd_preview_surface_shaders:
            return
        ...
        for usd_shade_input in usdshade_shader.GetInputs():
            ...
            if cls._should_input_be_filtered(usd_shade_input):
                # don't allow connections to any of these parameters
                cls.write_usd_shade_input_connectibility(usd_shade_input, UsdShade.Tokens.interfaceOnly)
                continue

            sdr_property = sdr_node.GetShaderInput(base_name) if USD_2508_API else sdr_node.GetInput(base_name)
            if not sdr_property:
                if cls._validate_usd_shade_input_invalid_connection(usd_shade_input):
                    cls.write_usd_shade_input_empty_connection(usd_shade_input)

                # don't allow connections to any of these parameters
                cls.write_usd_shade_input_connectibility(usd_shade_input, UsdShade.Tokens.interfaceOnly)
                continue

            if cls._validate_usd_shade_input(usd_shade_input, sdr_property) is not None:
                cls.update_usd_shade_input(usd_shade_input, sdr_property)
        ...
        for usdshade_output in usdshade_shader.GetOutputs():
            ...
            if cls._validate_usdshade_output(usdshade_output, sdr_property) is not None:
                cls.update_usdshade_output(usdshade_output, sdr_property)

    @classmethod
    def update_usdshade_nodegraph(cls, usdshade_nodegraph: UsdShade.NodeGraph) -> None:
        ...
        for usd_shade_input, connected_inputs in consumers_map.items():
            if connected_inputs:
                cls.write_usd_shade_input_output_type_name(usd_shade_input, connected_inputs[0].GetTypeName())

        for usdshade_output in usdshade_nodegraph.GetOutputs():
            connections = usdshade_output.GetConnectedSources()
            if connections[0]:
                cls.write_usd_shade_input_output_type_name(usdshade_output, connections[0][0].typeName)

    @classmethod
    def update_prim(cls, stage: Usd.Stage, prim: Usd.Prim) -> bool:
        ...
        if UsdShade.ConnectableAPI(prim).IsContainer():
            ...
            for child_prim in prim.GetAllChildren():
                did_update |= cls.update_prim(stage, child_prim)
            ...
            if did_update:
                ...
                if usdshade_nodegraph:
                    cls.update_usdshade_nodegraph(usdshade_nodegraph)

            return did_update

        elif cls._validate_prim(prim):
            usdshade_shader = UsdShade.Shader(prim)

            if usdshade_shader:
                cls.update_usdshade_shader(usdshade_shader)
    ...
    def _validate_attribute_type_name(
        ...
        if not cls._validate_type_name(sdr_property, type_name):
            sdf_type = get_sdf_type_for_shader_property(sdr_property)
            return cls._ValidationResult(
                requirement=cap.Requirements.VM_PS_001,
    ...
    def _validate_usd_shade_input_connections(
        ...
        is_connectable: bool = sdr_property.IsConnectable()

        if not is_connectable:
            source_info = connections[0][0]
            source_prim = source_info.source.GetPrim()
            # interfaceOnly inputs are allowed to be connected to node graphs (and therefore materials)
            # however Sdr still reports them as not-connectable as of USD 26.03, so we need to check explicitly.
            if source_info.sourceType == UsdShade.AttributeType.Input and source_prim.IsA(UsdShade.NodeGraph):
                is_connectable = True
    ...
    @classmethod
    def _validate_usd_shade_input_invalid_connection(
        cls, usd_shade_input: UsdShade.Input
    ) -> _ValidationResult | None:
        connections = usd_shade_input.GetConnectedSources()
        if not connections or not connections[0]:
            return
    ...
    def _validate_token_value(
        cls, usd_shade_input: UsdShade.Input, input_value: _InputValue, sdr_property: Sdr.ShaderProperty
    ) -> _ValidationResult | None:
        options = sdr_property.GetOptions()
    ...
    def _validate_usd_shade_input_value(
        ...
        if (input_value.value is not None) or (input_value.time_samples is not None):
            ...
            if sdf_type in [Sdf.ValueTypeNames.Token, Sdf.ValueTypeNames.String]:
                return cls._validate_token_value(usd_shade_input, input_value, sdr_property)
    ...
    def _validate_usd_shade_input(
        ...
        results = [
            cls._validate_attribute_type_name(sdr_property, usd_shade_input.GetAttr()),
            cls._validate_usd_shade_input_connections(usd_shade_input, sdr_property),
            cls._validate_usd_shade_input_connectability(usd_shade_input, sdr_property),
            cls._validate_usd_shade_input_metadata(usd_shade_input, sdr_property),
            cls._validate_usd_shade_input_value(usd_shade_input, input_value, sdr_property),
        ]
        return list(filter(None, results))

    @classmethod
    def _validate_usdshade_output(
        cls, usdshade_output: UsdShade.Output, sdr_property: Sdr.ShaderProperty
    ) -> _ValidationResult | None:
        return cls._validate_attribute_type_name(sdr_property, usdshade_output.GetAttr())

    @classmethod
    def _validate_usdshade_shader(
        cls, usdshade_shader: UsdShade.Shader, sdr_node: Sdr.ShaderNode
    ) -> list[_ValidationResult]:
        ...
        for usd_shade_input in usdshade_shader.GetInputs():
            ...
            sdr_property = sdr_node.GetShaderInput(base_name) if USD_2508_API else sdr_node.GetInput(base_name)
            if not sdr_property:
                results.append(cls._validate_usd_shade_input_invalid_connection(usd_shade_input))
                continue
            ...
            results.extend(cls._validate_usd_shade_input(usd_shade_input, sdr_property))
        ...
        for output_name in sdr_node.GetShaderOutputNames() if USD_2508_API else sdr_node.GetOutputNames():
            ...
            results.append(cls._validate_usdshade_output(usdshade_output, sdr_property))

        return list(filter(None, results))

    @classmethod
    def _validate_prim(cls, prim: Usd.Prim) -> list[_ValidationResult]:
        ...
        if UsdShade.ConnectableAPI(prim).IsContainer():
            results = []
            for child_prim in prim.GetAllChildren():
                results.extend(cls._validate_prim(child_prim))
            return results
        ...
        sdr_node = Sdr.Registry().GetShaderNodeByName(shader_id)
        return cls._validate_usdshade_shader(usdshade_shader, sdr_node)
...
@register_rule("Material")
@register_requirements(cap.Requirements.VM_MDL_002)
class MaterialOldMdlSchemaChecker(BaseRuleChecker):
    ...
    @staticmethod
    def update_deprecated_mdl_schema(_: Usd.Stage, prim: Usd.Prim) -> None:
        ...
        no update will occur and an exception will be raised.
        """

        shader = UsdShade.Shader(prim)
        if not shader:
            raise ValueError("Unable to apply deprecated MDL schema fix because the provided prim is not a shader.")
        ...
        if not module:
            errors.append("Missing or empty 'module' attribute.")
        elif module_attr.GetTypeName() != Sdf.ValueTypeNames.Asset:
            errors.append(f"'module' attribute not of expected type: '{Sdf.ValueTypeNames.Asset}'.")
            module = None
        ...
        raise ValueError(
            "Unable to fix shader using deprecated MDL schema due to the following:\n" + "\n".join(errors)
        )
    ...
    def CheckPrim(self, prim):
        ...
        if source_impl == "mdlMaterial":
            self._AddFailedCheck(
                requirement=cap.Requirements.VM_MDL_002,
