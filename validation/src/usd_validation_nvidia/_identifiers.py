...
from .utils import DedupeMetaclass, LocalUriResolver, deprecated, normalize_url
...
@dataclass(frozen=True)
class LayerId(Identifier[Sdf.Layer], metaclass=DedupeMetaclass):
    """
...
@dataclass(frozen=True)
class StageId(Identifier[Usd.Stage], metaclass=DedupeMetaclass):
    """
...
class PrimId(Identifier[Usd.Prim], VariantIdMixin):
    ...
    stage_id: StageId
    path: Sdf.Path
    spec_ids: SpecIdList = field(default_factory=lambda: EditTargetIdList())
...
class PropertyId(Identifier[Usd.Attribute], VariantIdMixin):
    ...
    def get_spec_ids(self) -> list[SpecId]:
        return [
            replace(node_id, path=node_id.path.AppendProperty(self.name)) for node_id in self.prim_id.get_spec_ids()
        ]
...
class EditTargetId(Identifier[Sdf.Layer | Sdf.Spec]):
    ...
    Attributes:
        ...
        composed (PrimId | PropertyId | None): Optional. A reference to the composed prim or property if this edit target is a composition arc.
    """

    layer_id: LayerId
    path: Sdf.Path | None = None
    composed: PrimId | PropertyId | None = None
...
@dataclass(frozen=True)
class EditTargetIdList(Sequence[EditTargetId]):
    """
    A list of edit target ids.
    """

    _values: list[EditTargetId] = field(default_factory=list, hash=False)

    def __len__(self) -> int:
        return len(self._values)

    def __getitem__(self, item: int) -> EditTargetId:
        return self._values[item]

    @property
    @deprecated("Use as a list instead")
    ...
    @from_.register(type(None))
    @classmethod
    def _(cls, obj: None) -> EditTargetIdList:
        return EditTargetIdList()

    @from_.register(Sdf.Layer)
    @classmethod
    def _(cls, layer: Sdf.Layer) -> EditTargetIdList:
        return EditTargetIdList([EditTargetId.from_(layer)])

    @from_.register(Sdf.Spec)
    @classmethod
    def _(cls, spec: Sdf.Spec) -> EditTargetIdList:
        return EditTargetIdList([EditTargetId.from_(spec)])
    ...
    def _(cls, prim: Usd.Prim) -> EditTargetIdList:
        ...
        for spec in prim.GetPrimStack():
            ...
            if edit_target_id._internal:
                continue
            edit_target_ids.append(replace(edit_target_id, composed=prim_id))
        return EditTargetIdList(edit_target_ids)
    ...
    def _(cls, prop: Usd.Property) -> EditTargetIdList:
        ...
        for spec in prop.GetPropertyStack(time=Usd.TimeCode.Default()):
            ...
            if edit_target_id._internal:
                continue
            edit_target_ids.append(replace(edit_target_id, composed=property_id))
        return EditTargetIdList(edit_target_ids)
    ...
    @from_.register(list)
    @classmethod
    def _(cls, objects: list[AtType]) -> EditTargetIdList:
        result: list[EditTargetId] = []
        for obj in objects:
            result.extend(EditTargetIdList.from_(obj))
        return EditTargetIdList(result)
...
class FormatDependencyId(Identifier[FormatDependency]):
    ...
    def restore(self, stage: Usd.Stage | None = None) -> FormatDependency:
        return FormatDependency(path=self.path, uri_resolver=LocalUriResolver(), root_asset_path=self.root_asset_path)
...
def _(value: list[AtType]) -> list[Identifier[AtType]]:
    return [to_identifier(item) for item in value]


@to_identifier.register(tuple)
def _(value: tuple[AtType, ...]) -> tuple[Identifier[AtType], ...]:
    return tuple(to_identifier(item) for item in value)


def to_identifiers(value: list[AtType] | None) -> list[Identifier[AtType]] | None:
    """Alias for :func:`to_identifier` when called with a list."""
    return to_identifier(value)


def _restore_path(path: str | None) -> Sdf.Path:
    return Sdf.Path.emptyPath if path is None else Sdf.Path(path)


def _reduce_path(path: Sdf.Path) -> tuple[Callable[[str | None], Sdf.Path], tuple[str | None]]:
    return _restore_path, (None if path == Sdf.Path.emptyPath else str(path),)


copyreg.pickle(Sdf.Path, _reduce_path)


@singledispatch
def restore_identifier(value: Any, stage: Usd.Stage | None = None) -> Any:
    return value


@restore_identifier.register(Identifier)
def _(value: Identifier[AtType], stage: Usd.Stage | None = None) -> AtType:
    return value.restore(stage)


@restore_identifier.register(str)
def _(value: str, stage: Usd.Stage | None = None) -> str:
    return value


@lru_cache(maxsize=1)
def _restore_stage(value: StageId) -> Usd.Stage:
    root_layer = Sdf.Layer.FindOrOpen(value.root_layer.identifier)
    return Usd.Stage.Open(root_layer)


@restore_identifier.register(StageId)
def _(value: StageId, stage: Usd.Stage | None = None) -> Usd.Stage:
    if stage is None:
        return _restore_stage(value)
    else:
        return value.restore(stage)


@restore_identifier.register(PrimId)
def _(value: PrimId, stage: Usd.Stage | None = None) -> Usd.Prim:
    if stage is None:
        stage = restore_identifier(value.stage_id)
    return value.restore(stage)


@restore_identifier.register(PropertyId)
def _(value: PropertyId, stage: Usd.Stage | None = None) -> Usd.Property:
    if stage is None:
        stage = restore_identifier(value.stage_id)
    return value.restore(stage)


@restore_identifier.register(SchemaBaseId)
def _(value: SchemaBaseId, stage: Usd.Stage | None = None) -> Usd.SchemaBase:
    if stage is None:
        stage = restore_identifier(value.prim_id.stage_id)
    return value.restore(stage)


@restore_identifier.register(FormatDependencyId)
def _(value: FormatDependencyId, stage: Usd.Stage | None = None) -> FormatDependency:
    return value.restore(stage)


@restore_identifier.register(list)
def _(value: list[Any], stage: Usd.Stage | None = None) -> list[Any]:
    return [restore_identifier(item, stage) for item in value]


@restore_identifier.register(tuple)
def _(value: tuple[Any, ...], stage: Usd.Stage | None = None) -> tuple[Any, ...]:
    return tuple(restore_identifier(item, stage) for item in value)
